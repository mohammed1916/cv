...
const ROOT_PATH = fileURLToPath(new URL("../src/problems/", import.meta.url));
...
const files = (await walk(ROOT_PATH)).filter(
  (path) =>
    /(?:^|[\\/])[^\\/]*Visualizer\.jsx$/.test(path) &&
...
for (const path of files) {
  ...
  const label = path.replace(ROOT_PATH, "");
  ...
  const hasExamples =
    /examples=|applyExample|getExamples(?:Or)?|\bEXAMPLES\b|\b(?:const|let)\s+examples\s*=|INPUT_PRESETS|handleExample(?:Load|Click)?/.test(
      source,
    );
  const hasPlayback = /<PlaybackControls\b|<VisualizerPlaybackSection\b/.test(
    source,
  );
  const hasFloatingPlayback =
    /<FloatingPanel\s+title=["']Playback Controls["']|<VisualizerPlaybackSection\b[\s\S]{0,400}?\bfloatingPlayback\b/.test(
      source,
    );
  const hasLumino = /<LuminoDockPanel\b/.test(source);
  const hasCanonicalLayout =
    /id:\s*["']input["']/.test(source) && /id:\s*["']code["']/.test(source);
  const canReadMissingStep =
    /const\s+step\s*=\s*steps\[currentStep\]\s*\|\|\s*steps\[0\]\s*;/.test(
      source,
    );
  const hasLegacyConnectivityCall =
    /useCodeVisualConnectivity\(\s*[^\{\s][^,)]*,\s*[^)]*\)/.test(source);
...
if (
  ...
  Object.values(missing).some((paths) => paths.length)
)
  process.exitCode = 1;
