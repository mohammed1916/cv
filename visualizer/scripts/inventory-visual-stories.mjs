...
const progress = fs.existsSync(progressPath)
  ? JSON.parse(fs.readFileSync(progressPath, "utf8")).entries
  : [];
...
function inspect(file) {
  ...
  try {
    ...
    const sources = tree.program.body
      .filter((node) =>
        [
          "ImportDeclaration",
    ...
    for (const source of sources) {
      ...
      if (resolved && /\.(jsx?|css)$/.test(resolved))
...
function collect(file, visited = new Set()) {
  ...
  visited.add(file);
  for (const dependency of inspect(file).dependencies)
    collect(dependency, visited);
  return visited;
}
const entries = [];
for (const entry of fs
  ...
  const ownFiles = reachable.filter((file) =>
    file.startsWith(`${directory}${path.sep}`),
  );
  const ownSource = ownFiles
    .filter((file) => !file.endsWith(".css"))
    .map((file) => inspect(file).source)
    .join("\n");
  ...
  entries.push({
    ...
    signals: {
      localSVG: /<svg\b/.test(ownSource),
      treeView: /TreeCanvas|TreeVisualizer/.test(source),
      sharedWorkspace: /<Algorithm(?:Story)?Workspace\b/.test(source),
      playback: /<PlaybackControls\b/.test(source),
      floating: /<FloatingPanel\b/.test(source),
      lumino: /<LuminoDockPanel\b/.test(source),
      emptyPatternDefinition: /const\s+PATTERNS\s*=\s*\[\s*\]/.test(ownSource),
    },
    parseErrors: reachable
      .filter((file) => inspect(file).parseError)
      .map((file) => ({
        file: relative(file),
        error: inspect(file).parseError,
      })),
  });
}
const cssFiles = fs
...
const report = {
  ...
  cssSourceBytes: cssFiles.reduce(
    (total, file) => total + fs.statSync(path.join(root, "src", file)).size,
    0,
  ),
  entries,
};
...
fs.writeFileSync(
  path.join(root, "docs/visual-story-inventory.json"),
  JSON.stringify(report, null, 2) + "\n",
);
const known = new Set(progress.map((entry) => entry.folder));
const additions = entries
  .filter((entry) => !known.has(entry.folder))
  .map(({ number, folder, title }) => ({
    number,
    folder,
    title,
    story: "pending-review",
    playground: "not-assessed",
  }));
if (additions.length)
  fs.writeFileSync(
    progressPath,
    JSON.stringify(
      {
        note: "Explicit review ledger. Inventory flags do not certify visual quality. Verified evidence is recorded in visual-story-plan.md.",
        entries: [...progress, ...additions],
      },
      null,
      2,
    ) + "\n",
...
console.log(
  `Inventoried ${entries.length} registered entries and ${cssFiles.length} CSS files. See docs/visual-story-inventory.json.`,
);
