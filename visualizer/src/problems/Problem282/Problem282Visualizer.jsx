...
import './Problem282Visualizer.css'
...
export default function Problem282Visualizer() {
    const examples = useMemo(() => getExamplesOr('282', []), [])
...
const applyEx = useCallback((i) => { setCurrentExample(i); setInputInput(JSON.stringify(examples[i].input)); setCurrentStep(0); }, [setCurrentStep]);
    ...
    return (
        <DockableWorkspace
            ...
            subtitle="expression-add-operators"
            accentColor="#06b6d4"
        >
        <ManualInputPanel
        ...
        />
            ...
            <FloatingPanel title="Visualization" position="main">
                <div className="problem282-visualizer-viz-panel">
                    <div className="problem282-visualizer-canvas">
                        <motion.div
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            transition={{ duration: 0.3 }}
                            className="problem282-visualizer-content"
                        >
                            <p>{step.message}</p>
                        </motion.div>
                    </div>
