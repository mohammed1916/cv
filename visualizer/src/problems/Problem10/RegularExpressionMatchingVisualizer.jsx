...
const SOLUTION_CODE = [
  ...
  {
    line: 10,
    text: "                matches = p[j - 1] == '.' or s[i - 1] == p[j - 1]",
  },
  { line: 11, text: "                if matches: dp[i][j] = dp[i - 1][j - 1]" },
  {
    line: 12,
    text: "                # dp[i][j] now holds the character/dot result",
  },
  { line: 13, text: "            else:" },
  {
    line: 14,
    text: "                # '*' can match 0 or more of the preceding element",
  },
  {
    line: 15,
    text: "                dp[i][j] = dp[i][j - 2]  # Case 1: zero occurrences",
  },
  {
    line: 16,
    text: "                prev_matches = p[j - 2] == '.' or s[i - 1] == p[j - 2]",
  },
  {
    line: 17,
    text: "                if prev_matches and dp[i - 1][j]: dp[i][j] = True",
  },
  {
    line: 18,
    text: "                # else dp[i][j] keeps the zero-occurrence result",
  },
  { line: 19, text: "" },
...
function generateSteps(s, p) {
    ...
    for (let j = 1; j <= n; j++) {
      ...
      if (p_char !== "*") {
        ...
        steps.push({
          ...
          activeLine: 16,
          message: `Case 2 (match x+): p[${j - 2}]='${p[j - 2]}' ${
            prevCharMatches ? "matches" : "does not match"
          } s[${i - 1}]='${s_char}'`,
          dpTable: dp.map((row) => [...row]),
        });
        ...
        if (prevCharMatches && dp[i - 1][j]) {
          ...
          steps.push({
            ...
            match: true,
            activeLine: 17,
            message: `Match found! dp[${i}][${j}] = true (dp[${
              i - 1
            }][${j}] was true)`,
            dpTable: dp.map((row) => [...row]),
          });
  ...
  steps.push({
    ...
    match: null,
    activeLine: 20,
    message: `Result: dp[${m}][${n}] = ${dp[m][n]}. Pattern ${
      dp[m][n] ? "matches" : "does not match"
    } string.`,
    dpTable: dp.map((row) => [...row]),
  });
...
function VisualizationPanel({ s, p, step }) {
  ...
  const raySources = useMemo(
    () => new Set(raySourceCoords.map(([row, column]) => `${row}-${column}`)),
    [raySourceCoords],
  );
  ...
  const rays = useMemo(() => {
    ...
    if (
      ...
      if (step.i >= 1) {
        ...
        if (multiFrom) {
          const multiKnown =
            step.phase === "star_multi" || step.phase === "star_result";
  ...
  return (
    ...
    >
      <div className="rem-panel-head">DP Table &amp; Strings</div>
          ...
          <div className="rem-string-row">
            ...
            <div className="rem-string-chars">
              {s.split("").map((char, idx) => {
                const isActive =
                  step && step.i === idx + 1 && step.phase !== "done";

                const isProcessed = step && step.i != null && step.i > idx + 1;
          ...
          <div className="rem-string-row">
            ...
            <div className="rem-string-chars">
              {p.split("").map((char, idx) => {
                const isActive =
                  step && step.j === idx + 1 && step.phase !== "done";

                const isProcessed = step && step.j != null && step.j > idx + 1;
              ...
              {step?.dpTable &&
                step.dpTable.map((row, i) => (
                  <div key={`row-${i}`} className="rem-dp-row">
                    ...
                    {row.map((val, j) => {
                      const isCurrent =
                        step.i === i && step.j === j && step.phase !== "done";

                      const isTarget =
                        step.phase === "done" && i === m && j === n;
...
export default function RegularExpressionMatchingVisualizer() {
  ...
  const { s, p, inputError } = useMemo(() => {
    try {
      ...
      if (sValue.length > 30 || pValue.length > 30) {
        return {
          s: "",
          p: "",
          inputError: "Input strings must be <= 30 characters",
        };
      }
  ...
  const vizFeatureDefs = getVisualizationFeatures(
  ...
  const mainPanel = (
    ...
    >
      ...
      */}
      ...
      >
        <VisualizationPanel s={s} p={p} step={step} />
      </div>
    </div>
  );
  ...
  const statusPanel = (
    ...
    >
      ...
      >
        {step
          ? step.phase === "done"
            ? `Match: ${step.dpTable[s.length][p.length] ? "Yes" : "No"}`
            : step.message
