...
function generateSteps(root) {
  ...
  while (q.length > 0) {
    ...
    steps.push({
      activeLine: 5,
      levelNum,
      message: `Process level ${levelNum} (${size} nodes)`,
      relatedLines: [5],
    })
    ...
    steps.push({
      ...
      connectedLevel: true,
      message: `Level ${levelNum} complete: [${currentLevel.join(' → ')}]`,
      relatedLines: [12],
    })
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 16 }}>
      <div style={{ padding: 12, backgroundColor: '#f3e8ff', borderRadius: 6, borderLeft: '4px solid #8b5cf6' }}>
        <div style={{ fontSize: 12, color: '#5b21b6', fontStyle: 'italic' }}>
          Handle arbitrary tree structure: only connect nodes that actually exist (handles null children).
        </div>
      </div>
...
export default function PopulatingNextRightPointersIIVisualizer() {
  const [input, setInput] = useState({"label":"Example 1","root":[1,2,3,4,5,null,7]});
  const [rootInput, setRootInput] = useState("[1,2,3,4,5,null,7]");
  const { root, inputError } = useMemo(() => {
    try {
      ...
      return { root: [1,2,3,4,5,null,7], inputError: e.message };
    }
