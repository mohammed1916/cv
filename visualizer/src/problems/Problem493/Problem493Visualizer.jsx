...
function VisualizationPanel({ nums, step, applyEx }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 16, overflow: 'auto' }}>
      ...
      <div>
        <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)', marginBottom: 8 }}>Array</div>
...
export default function Problem493Visualizer() {
  ...
  const { nums, inputError } = useMemo(() => {
    try {
      ...
      return { nums: "[1,3,2,3,1]", inputError: e.message };
    }
  }, [numsInput]);
  ...
  const panelConfigs = useMemo(() => [
    { id: 'code', title: 'Code' },
    { id: 'viz', title: '🔄 Reverse Pairs', dockMode: 'split-right' },
  ], [])
  const panelContents = useMemo(() => ({
    code: (<CodeTracePanel
        ...
        />),
    viz: (<VisualizationPanel nums={nums} step={step} applyEx={applyEx} />),
