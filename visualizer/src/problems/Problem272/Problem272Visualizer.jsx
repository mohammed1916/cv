...
import './Problem272Visualizer.css'
...
export default function Problem272Visualizer() {
    const examples = useMemo(() => getExamplesOr('272', []), [])
...
const applyEx = useCallback((i) => { setCurrentExample(i); setInputInput(JSON.stringify(examples[i].input)); setCurrentStep(0); }, [setCurrentStep]);
    ...
    return (
        <DockableWorkspace
            ...
            subtitle="closest-binary-search-tree-value-ii"
            accentColor="#3b82f6"
        >
        <ManualInputPanel
        ...
        />
            ...
            <FloatingPanel title="Visualization" position="main">
                <div className="problem272-visualizer-viz-panel">
                    <div className="problem272-visualizer-canvas">
                        <motion.div
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            transition={{ duration: 0.3 }}
                            className="problem272-visualizer-content"
                        >
                            <p>{step.message}</p>
                        </motion.div>
                    </div>
