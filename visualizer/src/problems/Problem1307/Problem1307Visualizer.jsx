...
export default function Problem1307Visualizer() {
  ...
  const { equation, inputError } = useMemo(() => {
    try {
      ...
      return { equation: parsedEquation, inputError: '' };
    } catch (e) {
      return { equation: "SEND+MORE=MONEY", inputError: e.message };
    }
  }, [equationInput]);
  const SOLUTION_CODE = SOLUTION_CODE_INLINE
  const steps = useMemo(
    () => generateSteps(equation).map((c) => ({
      ...c,
  ...
  const applyEx = useCallback((e) => { setEx(e); setEquationInput(String(e.equation)); handleReset(); }, [handleReset]);
  const connectivity = useCodeVisualConnectivity({ steps, stepIndex, onStepJump: setStepIndex })
  const { showPatternOverlay, setShowPatternOverlay, activeLineDom, setActiveLineDom } = usePatternOverlay()
  const panelConfigs = useMemo(() => [
    { id: 'code', title: 'Code' },
    { id: 'viz', title: '🔢 Cryptarithmetic', dockMode: 'split-right' },
  ], [])
