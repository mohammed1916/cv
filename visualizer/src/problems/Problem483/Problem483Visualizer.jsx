...
function VisualizationPanel({ n, step, applyEx }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, padding: 16 }}>
      ...
      <div>
        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--surface2)', marginBottom: 8 }}>Number: {n}</div>
      </div>
...
export default function Problem483Visualizer() {
  ...
  const panelConfigs = useMemo(() => [
    { id: 'code', title: 'Code' },
    { id: 'viz', title: '🔍 Smallest Good Base', dockMode: 'split-right' },
  ], [])
  const panelContents = useMemo(() => ({
    ...
    viz: (<VisualizationPanel n={parseInt(n)} step={step} applyEx={applyEx} />),
