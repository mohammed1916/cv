...
const SOLUTION_CODE = [
  ...
  {
    line: 3,
    text: "    # dp[a][l] = ways with a absences so far and a trailing run of l lates",
  },
  ...
  {
    line: 17,
    text: "                    nxt[1][0] = (nxt[1][0] + ways) % MOD",
  },
  { line: 18, text: "                # append L: only if run < 2" },
  { line: 19, text: "                if l < 2:" },
  {
    line: 20,
    text: "                    nxt[a][l + 1] = (nxt[a][l + 1] + ways) % MOD",
  },
  { line: 21, text: "        dp = nxt" },
  { line: 22, text: "" },
  {
    line: 23,
    text: "    return sum(dp[a][l] for a in range(2) for l in range(3)) % MOD",
  },
];
...
function generateSteps(nStr) {
  ...
  try {
    ...
    if (n > 40)
      throw new Error(
        "Keep n at 40 or below so the walkthrough stays readable",
      );
    ...
    for (let day = 1; day <= n; day += 1) {
      ...
      steps.push({
        ...
        activeLine: 20,
        message:
          "Appending L extends the run, but only from l < 2 — three in a row is banned.",
        n,
        day,
        dp: cloneDp(dp),
        next: cloneDp(nxt),
...
export default function StudentAttendanceRecordIIVisualizer() {
  ...
  const inputError = useMemo(() => {
    ...
    if (val > 40)
      return "Keep n at 40 or below so the walkthrough stays readable";
    return "";
  }, [nInput]);
  ...
  const renderDp = (table, changed) => (
    <div className="p552-dp">
      ...
      {RUN_LABELS.map((label) => (
        <div key={label} className="p552-dp-head">
          {label}
        </div>
      ))}
      {[0, 1].map((a) => (
  ...
  const primaryPanel = (
    <div className="p552-panel-primary">
            ...
            const cls =
              dayNo === step?.day
                ...
                : dayNo < (step?.day ?? 0)
                  ...
                  : "";
            return (
              <div key={idx} className={`p552-day ${cls}`}>
                {dayNo}
              </div>
            );
          })}
      ...
      <div className="p552-card">
        ...
        {step?.dp ? (
          renderDp(step.dp, step.next ? null : step.changed)
        ) : (
          <p className="p552-hint">—</p>
        )}
      </div>
    ...
    <div className="p552-panel-state">
      <div className="p552-card">
        ...
        <div className="p552-stat-grid">
          <div className="p552-stat">
            <span className="p552-stat-key">n</span>
            <span className="p552-stat-val">{step?.n ?? "—"}</span>
          </div>
          <div className="p552-stat highlight">
            <span className="p552-stat-key">day</span>
            <span className="p552-stat-val">{step?.day ?? "—"}</span>
          </div>
          <div className="p552-stat">
            <span className="p552-stat-key">total</span>
            <span className="p552-stat-val">
              {step?.dp
                ? step.dp.flat().reduce((s, v) => (s + v) % MOD, 0)
                : "—"}
            </span>
          </div>
          <div className="p552-stat">
            <span className="p552-stat-key">mod</span>
            <span className="p552-stat-val">1e9+7</span>
          </div>
        </div>
      </div>
      ...
      <div className="p552-card">
        ...
        <p className="p552-hint" style={{ marginTop: 0 }}>
          <strong>a</strong> — absences used so far (0 or 1).
          <br />
          <strong>l</strong> — length of the trailing run of L (0, 1 or 2).
          <br />A record is rewardable exactly when it never needs a = 2 or l =
          3.
        </p>
      </div>
    </div>
  ...
  const statusPanel = (
    <div className="p552-panel-status">
      <div
        className={`p552-status ${step?.phase === "done" ? "done" : step?.error ? "error" : ""}`}
      >
