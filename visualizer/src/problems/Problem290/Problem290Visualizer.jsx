...
import './Problem290Visualizer.css'
...
export default function Problem290Visualizer() {
    const examples = useMemo(() => getExamplesOr('290', []), [])
...
const applyEx = useCallback((i) => { setCurrentExample(i); setInputInput(JSON.stringify(examples[i].input)); handleReset(); }, [handleReset]);
    ...
    return (
        <DockableWorkspace
            title="Word Pattern"
            subtitle="word-pattern"
            accentColor="#10b981"
        >
      
            <FloatingPanel title="Visualization" position="main">
                <div className="problem290-visualizer-viz-panel">
                    <div className="problem290-visualizer-canvas">
                        <motion.div
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
