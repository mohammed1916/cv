...
function generateSteps(arr) {
  ...
  snap(3, "Find middle with slow/fast pointers.", {
    slow: 0,
    fast: 1,
    phase: "find_mid",
  });
  ...
  while (fi < list.length && fi + 1 < list.length) {
    ...
    snap(5, `slow→${si} (val=${list[si]}), fast→${fi}`, {
      slow: si,
      fast: fi,
      phase: "find_mid",
    });
  ...
  // si is mid index
  const mid = si;
  snap(6, `Middle is index ${mid}. Split list into two halves.`, {
    slow: mid,
    fast: -1,
    phase: "split",
  });
  ...
  snap(8, `Reverse second half: [${secondHalf.join("→")}]`, {
    firstHalf: [...firstHalf],
  ...
  for (let i = 0; i < reversed.length; i++) {
    snap(11, `Reversed so far: [${reversed.slice(0, i + 1).join("→")}]`, {
      firstHalf: [...firstHalf],
      secondHalf: reversed.slice(0, i + 1),
      phase: "reversing",
    });
  }
  ...
  snap(
    13,
    `Merge first=[${firstHalf.join("→")}] and reversed=[${reversed.join("→")}]`,
    {
      firstHalf: [...firstHalf],
      secondHalf: [...reversed],
      phase: "merge_start",
  ...
  while (s < reversed.length) {
    ...
    snap(16, `Take from first: ${firstHalf[f]}`, {
      result: [...result, ...firstHalf.slice(f + 1), ...reversed.slice(s)],
      mergeF: f,
      mergeS: s,
      phase: "merging",
    });
    ...
    snap(16, `Take from second: ${reversed[s]}`, {
      result: [...result, ...firstHalf.slice(f), ...reversed.slice(s + 1)],
      mergeF: f,
      mergeS: s,
      phase: "merging",
    });
...
export default function ReorderListVisualizer() {
  ...
  const [arrInput, setArrInput] = useState(
    JSON.stringify(EXAMPLES[0]?.["arr"] ?? null),
  );
  const { arr, inputError } = useMemo(() => {
    try {
      const parsedArr = JSON.parse(arrInput);
      if (!Array.isArray(parsedArr)) throw new Error("arr must be an array");
  ...
  const applyExample = useCallback(
    (i) => {
      setSel(i);
      setArrInput(JSON.stringify(EXAMPLES[i].arr));
      handleReset();
    },
    [handleReset],
  );
        ...
        {step?.firstHalf && (
          ...
          >
            ...
            >
              ...
              >
                Second {step.phase?.includes("revers") ? "(reversed)" : ""}
              </div>
