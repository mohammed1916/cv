...
export default function GuessNumberVisualizer() {
  ...
  const { n, pick, inputError } = useMemo(() => {
    try {
      ...
      if (isNaN(pv) || pv < 1 || pv > nv)
        throw new Error(`pick must be between 1 and ${nv}`);
  ...
  return (
    <div className="gn-shell">
      <ManualInputPanel
        fields={[
          { key: "n", label: "n", type: "string" },
          { key: "pick", label: "pick", type: "string" },
        ]}
        values={{ n: nInput, pick: pickInput }}
        onChange={(k, v) => {
          if (k === "n") setNInput(v);
          if (k === "pick") setPickInput(v);
          handleReset();
        }}
        examples={EXAMPLES}
        ...
        <div className="gn-body">
            ...
            <div className="gn-bar-wrap">
              ...
              <AnimatePresence>
                {mid !== null && (
                  ...
                  >
                    <div
                      className={`gn-marker-pin mid-pin${found ? " mid-found" : res === -1 ? " mid-high" : res === 1 ? " mid-low" : ""}`}
                    />
          ...
          <AnimatePresence mode="wait">
            {guessLabel && (
              ...
              >
                {res === 0 &&
                  `Correct! Found ${mid} in ${steps.filter((s) => s.phase === "found" || s.phase === "guess_call").length - 1} guess(es).`}
                {res === -1 &&
                  `guess(${mid}) = -1 → ${mid} is too high, secret is lower. New hi = ${hi}.`}
                {res === 1 &&
                  `guess(${mid}) = +1 → ${mid} is too low, secret is higher. New lo = ${lo}.`}
              </motion.div>
            )}
          </AnimatePresence>
