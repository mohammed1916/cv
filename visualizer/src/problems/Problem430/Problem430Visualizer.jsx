...
export default function Problem430Visualizer() {
  ...
  const steps = useMemo(
    () => generateSteps(structure).map(c => ({ ...c, relatedLines: c.relatedLines ?? (c.activeLine != null ? [c.activeLine] : []) })),
    [structure]
  )
  ...
  const panelConfigs = useMemo(() => [
    { id: 'code', title: 'Code' },
    { id: 'viz', title: '🔗 Flatten Multi-level', dockMode: 'split-right' },
  ], [])
  ...
  return (
    <div className="problem-shell">
        <ManualInputPanel
          fields={[{"key":"structure","label":"structure","type":"string"}]}
          values={{ structure: structureInput }}
          onChange={(k, v) => { if (k === 'structure') setStructureInput(v); handleReset() }}
          examples={EXAMPLES}
