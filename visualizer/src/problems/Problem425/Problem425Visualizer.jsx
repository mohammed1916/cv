...
function WordSquareVisualization({ square }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>Word Square</div>
      {square.length > 0 ? (
        <div style={{
...
export default function Problem425Visualizer() {
  ...
  const panelConfigs = useMemo(() => [
    { id: 'code', title: 'Code' },
    { id: 'viz', title: '🔤 Word Squares', dockMode: 'split-right' },
  ], [])
  const panelContents = useMemo(() => ({
    code: (<CodeTracePanel
        ...
        />),
    viz: (<VisualizationPanel words={words} step={step} applyEx={applyEx} />),
