...
const SOLUTION_CODE = getSolutionCode('matchsticks-to-square')
...
function VisualizationPanel({ nums, step, applyEx }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, padding: 16 }}>
      ...
      <div>
        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', marginBottom: 8 }}>Square Sides</div>
...
export default function Problem473Visualizer() {
  ...
  const [numsInput, setNumsInput] = useState("[1,1,2,2,2]");
  const { nums, inputError } = useMemo(() => {
    try {
      const parsedNums = JSON.parse(numsInput); if (!Array.isArray(parsedNums)) throw new Error('nums must be an array');
      return { nums: parsedNums, inputError: '' };
    } catch (e) {
      return { nums: "[1,1,2,2,2]", inputError: e.message };
    }
  }, [numsInput]);
  ...
  const panelConfigs = useMemo(() => [
    { id: 'code', title: 'Code' },
    { id: 'viz', title: '📏 Matchsticks to Square', dockMode: 'split-right' },
  ], [])
