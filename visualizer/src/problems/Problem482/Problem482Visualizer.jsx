  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 16, overflow: 'auto' }}>
      ...
      {step?.cleaned && step.cleaned.length > 0 && (
        <div>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)', marginBottom: 8 }}>After Cleaning</div>
      ...
      {step?.groups && step.groups.length > 0 && (
        <div>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)', marginBottom: 8 }}>Groups Formed</div>
...
export default function Problem482Visualizer() {
  ...
  const { s, k, inputError } = useMemo(() => {
    try {
      ...
      return { s: parsedS, k: parsedK, inputError: '' };
    } catch (e) {
      return { s: "5F3Z-2e-9-w", k: 4, inputError: e.message };
    }
  }, [sInput, kInput]);
  const SOLUTION_CODE = SOLUTION_CODE_INLINE

  const steps = useMemo(
    () => generateSteps(s, k).map(c => ({ ...c, relatedLines: c.relatedLines ?? (c.activeLine != null ? [c.activeLine] : []) })),
    [s, k]
  )

  const { stepIndex, setStepIndex, stepForward, stepBack, togglePlay, handleReset, isPlaying, speed, setSpeed, isDone } = usePlaybackState(steps.length)
  const step = stepIndex >= 0 ? steps[stepIndex] : null
  const applyEx = useCallback((e) => { setEx(e); setSInput(String(e.s)); setKInput(String(e.k)); handleReset(); }, [handleReset]);
  ...
  const panelConfigs = useMemo(() => [
    { id: 'code', title: 'Code' },
    { id: 'viz', title: '🔑 License Key Format', dockMode: 'split-right' },
  ], [])
  const panelContents = useMemo(() => ({
    code: (<CodeTracePanel
        ...
        />),
    viz: (<VisualizationPanel s={s} k={k} step={step} applyEx={applyEx} />),
