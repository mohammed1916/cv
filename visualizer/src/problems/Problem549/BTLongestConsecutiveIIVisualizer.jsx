...
const SOLUTION_CODE = [
  ...
  {
    line: 5,
    text: "        if not node: return (0, 0)   # (inc, dec) lengths",
  },
  ...
  {
    line: 11,
    text: "            if node.left.val == node.val + 1: inc = li + 1",
  },
  {
    line: 12,
    text: "            if node.left.val == node.val - 1: dec = ld + 1",
  },
  { line: 13, text: "        if node.right:" },
  {
    line: 14,
    text: "            if node.right.val == node.val + 1: inc = max(inc, ri + 1)",
  },
  {
    line: 15,
    text: "            if node.right.val == node.val - 1: dec = max(dec, rd + 1)",
  },
  { line: 16, text: "" },
  { line: 17, text: "        nonlocal best" },
  {
    line: 18,
    text: "        best = max(best, inc + dec - 1)  # child-parent-child path",
  },
    ...
    const dfs = (node) => {
      ...
      if (node.left) {
        if (node.left.val === node.val + 1) {
          inc = left.inc + 1;
          notes.push(
            `left ${node.left.val} = ${node.val}+1 → inc = ${left.inc}+1 = ${inc}`,
          );
        }
        if (node.left.val === node.val - 1) {
          dec = left.dec + 1;
          notes.push(
            `left ${node.left.val} = ${node.val}-1 → dec = ${left.dec}+1 = ${dec}`,
          );
        }
      }
      if (node.right) {
...
export default function BTLongestConsecutiveIIVisualizer() {
  ...
  const edges = useMemo(() => {
    ...
    const walk = (node) => {
      if (!node) return;
      if (node.left) {
        out.push([node.id, node.left.id]);
        walk(node.left);
      }
      if (node.right) {
        out.push([node.id, node.right.id]);
        walk(node.right);
      }
    };
    walk(tree.root);
  ...
  const primaryPanel = (
    <div className="p549-panel-primary">
      ...
      <div className="p549-card">
              ...
              {tree.nodes.map((node) => {
                ...
                return (
                  <g key={node.id} className={cls}>
                    ...
                    <text className="p549-node-val" x={pos.x} y={pos.y}>
                      {node.val}
                    </text>
                    {runs && (
                      <text className="p549-node-runs" x={pos.x} y={pos.y + 28}>
                        ({runs})
                      </text>
                    )}
                  </g>
        ...
        <p className="p549-hint">
          Labels below nodes show the returned (inc, dec) run lengths.
        </p>
      </div>
    ...
    <div className="p549-panel-state">
      <div className="p549-card">
        ...
        <div className="p549-stat-grid">
          <div className="p549-stat">
            <span className="p549-stat-key">inc</span>
            <span className="p549-stat-val">{step?.inc ?? "—"}</span>
          </div>
          <div className="p549-stat">
            <span className="p549-stat-key">dec</span>
            <span className="p549-stat-val">{step?.dec ?? "—"}</span>
          </div>
          <div className="p549-stat">
            <span className="p549-stat-key">inc+dec-1</span>
            <span className="p549-stat-val">{step?.candidate ?? "—"}</span>
          </div>
          <div className="p549-stat highlight">
            <span className="p549-stat-key">best</span>
            <span className="p549-stat-val">{step?.best ?? 0}</span>
          </div>
        </div>
      </div>
          ...
          <table className="p549-table">
            ...
            <tbody>
              {Object.entries(step.resolved).map(([id, info]) => (
                <tr
                  key={id}
                  className={step.current === Number(id) ? "current" : ""}
                >
                  <td>{info.val}</td>
                  <td>{info.inc}</td>
                  <td>{info.dec}</td>
  ...
  const statusPanel = (
    <div className="p549-panel-status">
      <div
        className={`p549-status ${step?.phase === "done" ? "done" : step?.error ? "error" : ""}`}
      >
