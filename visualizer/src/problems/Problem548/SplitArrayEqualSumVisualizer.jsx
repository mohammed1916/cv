...
const SOLUTION_CODE = [
  ...
  {
    line: 12,
    text: "                seen.add(pre[i])         # equal left pair",
  },
  { line: 13, text: "        for k in range(j + 2, n - 1):    # right cut" },
  {
    line: 14,
    text: "            if pre[n] - pre[k + 1] == pre[k] - pre[j + 1]:",
  },
...
function parseNums(text) {
  ...
  if (nums.length > 24)
    throw new Error("Keep the array to 24 numbers or fewer");
  return nums;
}
...
function generateSteps(input) {
  ...
  try {
    ...
    for (let j = 3; j < n - 3; j += 1) {
      ...
      for (let k = j + 2; k < n - 1; k += 1) {
        ...
        if (hit) {
          ...
          let foundI = null;
          for (let i = 1; i < j - 1; i += 1) {
            if (pre[i] === part3 && pre[j] - pre[i + 1] === part3) {
              foundI = i;
              break;
            }
          }
    ...
    steps.push({
      ...
      activeLine: 17,
      message:
        "Every middle cut exhausted with no matching quadruple split. Answer: false",
      nums,
      pre,
      result: false,
    });
...
export default function SplitArrayEqualSumVisualizer() {
    ...
    <div className="p548-panel-primary">
      ...
      <div className="p548-card">
        ...
        <p className="p548-hint">
          Struck-through cells are the excluded cut elements nums[i], nums[j],
          nums[k].
        </p>
      </div>
        ...
        <div className="p548-card">
          ...
          <div className="p548-parts">
            {step.parts.map((sum, idx) => {
              const allEqual = step.parts.every(
                (s) => s != null && s === step.parts[0],
              );
              const pairEqual = idx < 2 ? step.leftMatch : step.rightMatch;
              const cls =
                sum == null ? "" : allEqual || pairEqual ? "match" : "mismatch";
              return (
    ...
    <div className="p548-panel-state">
      <div className="p548-card">
        ...
        <div className="p548-stat-grid">
          <div className="p548-stat">
            <span className="p548-stat-key">i</span>
            <span className="p548-stat-val">{step?.i ?? "—"}</span>
          </div>
          <div className="p548-stat highlight">
            <span className="p548-stat-key">j</span>
            <span className="p548-stat-val">{step?.j ?? "—"}</span>
          </div>
          <div className="p548-stat">
            <span className="p548-stat-key">k</span>
            <span className="p548-stat-val">{step?.k ?? "—"}</span>
          </div>
        </div>
      </div>

      <div className="p548-card">
        <div className="p548-section-label">
          Left-Half Sums Seen (for this j)
        </div>
        {step?.seen?.length ? (
          <div className="p548-set">
            {step.seen.map((s) => (
              <span
                key={s}
                className={`p548-chip ${step.setHit === s ? "hit" : ""}`}
              >
                {s}
              </span>
            ))}
  ...
  const statusPanel = (
    <div className="p548-panel-status">
      <div
        className={`p548-status ${step?.phase === "done" ? "done" : step?.error ? "error" : ""}`}
      >
