...
function getCodeHighlight(step) {
  ...
  if (step.phase === "early-return")
    return { activeLine: 4, relatedLines: [3, 4] };
...
function generateZigzagSteps(input, numRows) {
    ...
    return [
      {
        ...
        placements: input
          .split("")
          .map((char, index) => ({ char, row: 0, col: index })),
        output: input,
        description:
          numRows === 1
  ...
  for (let index = 0; index < input.length; index++) {
    ...
    const nextRow =
      index === input.length - 1 ? currentRow : currentRow + nextDirection;
    const snapshot = {
      phase: "walk",
      char,
      charIndex: index,
...
function getRowSummary(rowStrings, previousStep, step, numRows) {
  const previousRows =
    previousStep?.rowStrings ?? Array.from({ length: numRows }, () => "");
...
function ZigzagGrid({ numRows, input, step }) {
  ...
  const placementMap = new Map(
    placements.map((item) => [`${item.row}-${item.col}`, item]),
  );
...
function ZigzagVisualizationPanel({
  numRows,
  source,
  currentStep,
  previousStep,
  ...
  const rowSummary = getRowSummary(
    currentStep?.rowStrings ?? Array.from({ length: numRows }, () => ""),
    previousStep,
    currentStep,
    numRows,
  );
      ...
      <div className="zv-card">
        <div className="zv-card-head">
          <div>
            ...
            <div className="zv-subtitle">
              Characters move down, then diagonally up, then repeat.
            </div>
          </div>
          <div className="zv-output-preview">
            <span className="zv-output-label">Output</span>
            <span className="mono zv-output-text">
              {currentStep?.output ?? ""}
            </span>
          </div>
        </div>
      ...
      <div className="zv-card zv-state-card">
        ...
        <div className="zv-state-grid">
          <div className="zv-state-item">
            ...
            <span className="zv-state-value mono">
              {currentStep?.char ?? "—"}
            </span>
          </div>
          <div className="zv-state-item">
          ...
          <div className="zv-state-item wide">
            ...
            <span className="zv-state-value">
              {currentStep?.description ??
                "Start the walkthrough to see each character placement."}
            </span>
          </div>
        </div>
      </div>
...
export default function ZigzagVisualizer() {
  ...
  const [steps, setSteps] = useState(() =>
    generateZigzagSteps(DEFAULT_INPUT, DEFAULT_ROWS),
  );
  ...
  const rowsAreValid =
    Number.isInteger(parsedRows) && parsedRows >= 1 && parsedRows <= 1000;
  const hasInput = sanitizedInput.trim().length > 0;
  const inputError =
    attemptedSubmit && !hasInput ? "Enter a string to visualize." : null;
  const rowError =
    attemptedSubmit && !rowsAreValid
      ? "Rows must be an integer between 1 and 1000."
      : null;
          ...
          {EXAMPLES.map((example) => (
            ...
            >
              <span className="zv-example-top">
                ...
                <span className="zv-example-chip mono">
                  {example.value} · {example.rows}r
                </span>
              </span>
