...
function generateSteps(ops) {
  ...
  function addIdx(val, i) {
    idx[val] = [...getIdx(val), i];
  }
  function popIdx(val) {
    const arr = idx[val] ?? [];
  ...
  function removeIdxVal(val, i) {
    idx[val] = getIdx(val).filter((x) => x !== i);
  }
  ...
  steps.push({
    ...
    result: null,
    phase: "init",
    message: "RandomizedCollection initialized.",
  });
  ...
  for (const op of ops) {
    if (op.type === "insert") {
      ...
      steps.push({
        ...
        result: isFirst,
        phase: "insert",
        message: `insert(${op.val}) → nums=[${nums.join(",")}], return ${isFirst}`,
      });
      ...
      if (getIdx(op.val).length === 0) {
        steps.push({
          ...
          result: false,
          phase: "miss",
          message: `remove(${op.val}) → not found`,
        });
        continue;
      }
      ...
      steps.push({
        ...
        swapI: i,
        swapLast: last,
        message: `remove(${op.val}): swap pos ${i} with last(${last}), pop → nums=[${nums.join(",")}]`,
      });
    } else {
      ...
      steps.push({
        ...
        result: pick,
        phase: "random",
        message: `getRandom() → ${pick}  (from [${nums.join(",")}])`,
      });
    }
  }
    ...
    try {
      ...
      if (
        ...
        ops.some(
          (op) =>
            !op ||
            !["insert", "remove", "getRandom"].includes(op.type) ||
            (op.type !== "getRandom" && !Number.isFinite(Number(op.val))),
        )
      )
        throw new Error(
          'Use {"ops":[{"type":"insert|remove","val":number}|{"type":"getRandom"}]}.',
        );
      return {
        ex: {
          ops: ops.map((op) =>
            op.type === "getRandom"
              ? { type: op.type }
              : { type: op.type, val: Number(op.val) },
          ),
        },
        inputError: "",
      };
    } catch (error) {
      return { ex: EXAMPLES[0], inputError: error.message };
    }
  }, [inputText]);
        ...
        >
            ...
            {EXAMPLES.map((e) => (
              <button
                ...
                style={{
                  ...
                  fontSize: 12,
                  backgroundColor:
                    ex.label === e.label ? "#dbeafe" : "var(--surface2)",
                }}
              >
                {e.label}
              </button>
          ...
          >
            ...
            >
              ...
              <div
                style={{ fontSize: 13, fontWeight: "bold", color: "#1e40af" }}
              >
                {opStr}
              </div>
            </div>
          ...
          <div
            style={{ fontSize: 12, fontWeight: 600, color: "var(--surface2)" }}
          >
            nums array
          </div>
          <div
              ...
              {nums.map((v, i) => (
                <motion.div
                  ...
                  style={{
                    ...
                    backgroundColor: i === step?.swapI ? "#fbbf24" : "#dbeafe",
                    border:
                      i === step?.swapI
                        ? "2px solid #f59e0b"
                        : "1px solid #0ea5e9",
          ...
          >
            ...
            {Object.values(idx).every((v) => v.length === 0) && (
              <span style={{ color: "var(--text-muted)", fontSize: 12 }}>
                empty
              </span>
            )}
          ...
          >
            {steps
              .slice(0, stepIndex + 1)
              .filter((s) => s.phase !== "init" && s.phase !== "done")
              .map((s, i) => (
                <span
                  key={i}
                  style={{
          ...
          <>
            {panelDivs.input &&
              createPortal(
                <ManualInputPanel
                  fields={[
                    { key: "ops", label: "Operations (JSON)", type: "string" },
                  ]}
                  values={{ ops: inputText }}
                  onChange={(_, value) => {
                    setInputText(value);
                    handleReset();
                  }}
