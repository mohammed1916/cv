  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 16, height: '100%', overflow: 'auto' }}>
          ...
          {KEYBOARD_ROWS.map((row, idx) => (
            <div
              ...
              style={{
                ...
                borderColor: step?.currentRow === idx ? row.color : 'var(--text-muted)',
                fontFamily: 'monospace',
                fontSize: 12,
                color: step?.currentRow === idx ? row.color : 'var(--text-muted)',
      ...
      {step?.result && (
        ...
        >
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 8 }}>Total Words</div>
...
export default function KeyboardRowVisualizer() {
  ...
  const panelConfigs = useMemo(() => [
    { id: 'code', title: 'Code' },
    { id: 'viz', title: '⌨ Keyboard Row', dockMode: 'split-right' },
  ], [])
  const panelContents = useMemo(() => ({
    ...
    viz: (<VisualizationPanel words={words} step={step} applyExample={applyExample} examples={examples} />),
