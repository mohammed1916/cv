...
export default function SummaryRanges() {
  ...
  const panelContents = {
    left: (<div className="vis-panel summary-ranges-panel">
            <div className="vis-panel-head summary-ranges-panel-head">
              Input
              {inputError && <span style={{ color: '#ea0c0c', marginLeft: 8 }}>{inputError}</span>}
            </div>
            <div className="vis-panel-body summary-ranges-panel-body">
              <div style={{ display: 'flex', gap: 6, marginBottom: 16, flexWrap: 'wrap' }}>
                {EXAMPLES.map((ex) => (
                  <button
                    key={ex.label}
                    onClick={() => applyExample(ex)}
                    className="vis-example-btn summary-ranges-example-btn"
                  >
                    {ex.label}
                  </button>
                ))}
    ...
    right: (<div className="vis-panel summary-ranges-panel">
            <div className="vis-panel-head summary-ranges-panel-head">Details</div>
            <div className="vis-panel-body summary-ranges-panel-body">
