    ...
    <div className="subsets-viz-panel">
      <div className="subsets-top">
          ...
          <div className="subsets-body">
            ...
            <div className="subsets-nums-row">
              {nums.map((v, i) => {
                ...
                const isPast =
                  step?.path?.length > 0 && i < (step?.start ?? nums.length);
                return (
                  <div
                    key={i}
                    className={`subsets-num-cell ${isStart ? "start" : ""} ${isPast ? "past" : ""}`}
                  >
        ...
        <section className="subsets-panel side">
          <header className="subsets-head">
            <span>Results ({step?.res?.length ?? 0})</span>
          </header>
          <div className="subsets-body">
            <div className="subsets-res-list">
              <AnimatePresence mode="popLayout">
                {(step?.res ?? []).map((subset, i) => (
                  <motion.div
                    key={i}
                    className={`subsets-res-item ${i === (step?.res?.length ?? 0) - 1 && step?.phase === "record" ? "latest" : ""}`}
