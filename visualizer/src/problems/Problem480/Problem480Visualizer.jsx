...
function VisualizationPanel({ nums, k, step, applyEx }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, padding: 16 }}>
      ...
      <div>
        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', marginBottom: 8 }}>Input Array (k={k})</div>
...
export default function Problem480Visualizer() {
  ...
  const { nums, k, inputError } = useMemo(() => {
    try {
      ...
      return { nums: "[1,3,-1,-3,5,3,6,7]", k: 3, inputError: e.message };
    }
  }, [numsInput, kInput]);
  const SOLUTION_CODE = SOLUTION_CODE_INLINE

  const steps = useMemo(() => generateSteps(nums, k).map(c => ({ ...c, relatedLines: c.relatedLines ?? (c.activeLine != null ? [c.activeLine] : []) })), [nums, k])
  ...
  const panelConfigs = useMemo(() => [
    { id: 'code', title: 'Code' },
    { id: 'viz', title: '📊 Sliding Window Median', dockMode: 'split-right' },
  ], [])
  const panelContents = useMemo(() => ({
    ...
    viz: (<VisualizationPanel nums={nums} k={k} step={step} applyEx={applyEx} />),
