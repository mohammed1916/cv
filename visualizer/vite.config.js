          ...
          try {
            ...
            const payload =
              provider === "gemini"
                ? {
                    ...
                    systemInstruction: {
                      parts: [
                        {
                          text:
                            messages.find((m) => m.role === "system")?.text ||
                            "",
                        },
                      ],
                    },
                ...
                : {
                    model: model || env.OLLAMA_MODEL || "gemma2:2b",
            ...
            const cloudApiKey =
              ollamaApiKey || env.OLLAMA_API_KEY || env.OLLAMA_CLOUD_API_KEY;
            if (provider === "ollama-cloud" && !cloudApiKey)
  ...
  return {
    ...
    build: {
      rolldownOptions: {
        output: {
          codeSplitting: {
            // Keep shared UI dependencies cacheable without combining the
            ...
            groups: [
              ...
              { name: "katex-vendor", test: /node_modules[\\/]katex[\\/]/ },
              {
                ...
                test: /node_modules[\\/](react-markdown|remark-[^\\/]+|rehype-[^\\/]+|micromark[^\\/]*|mdast-util-[^\\/]+|hast-util-[^\\/]+|unist-util-[^\\/]+|unified|vfile[^\\/]*)[\\/]/,
              },
            ],
          },
        },
